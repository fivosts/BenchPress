...
def http_get(url, temp_file, proxies=None, resume_size=0, user_agent: Union[Dict, str, None] = None):
  ...
  progress = tqdm(
    ...
    initial=resume_size,
    desc="Downloading",
    disable=bool(l.getLogger().getEffectiveLevel() == logging.NOTSET),
  )
...
def get_from_cache(
  ...
  with FileLock(lock_path):
    ...
    with temp_file_manager() as temp_file:
      l.getLogger().info("%s not found in cache or force_download set to True, downloading to %s", url, temp_file.name)

      http_get(url, temp_file, proxies=proxies, resume_size=resume_size, user_agent=user_agent)

    l.getLogger().info("storing %s in cache at %s", url, cache_path)
    os.replace(temp_file.name, cache_path)

    l.getLogger().info("creating metadata file for %s", cache_path)
