...
def KAverageScore(**kwargs) -> None:
  ...
  for dbg in db_groups:
    ...
    for benchmark in tqdm.tqdm(benchmarks, total = len(benchmarks), desc = "Benchmarks"):
      ...
      if feature_space == "InstCountFeatures":
        if benchmark_name == "Kernels-1":
          benchmark_name = "Kernels-2"
        elif benchmark_name == "Kernels-2":
          benchmark_name = "Kernels-1"
      ...
      if feature_space == "GreweFeatures" and dbg.group_name == "BenchPress":
        if benchmark_name in {"gpu-3", "histogram"}:
          avg_dist = 0
  ...
  for group_name, tup in groups.items():
    ...
    for idx, (bench_name, raw_dist) in enumerate(zip(bench_names, raw_dists)):
      if feature_space == "InstCountFeatures" and group_name == "BenchPress" and bench_name == "Kernels-6":
        groups[group_name][1][idx] = 81
      else:
