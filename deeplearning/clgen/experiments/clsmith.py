...
class CLSmithDatabase(sqlutil.Database):
  ...
  def get_features(self, sequence_length: int = None) -> typing.List[str]:
    ...
    with self.Session() as session:
      if sequence_length:
        return [x.feature_vector for x in session.query(CLSmithSample).filter(CLSmithSample.num_tokens <= sequence_length).limit(1000).offset(0).all()]
      else:
        return [x.feature_vector for x in session.query(CLSmithSample).limit(1000).offset(0).all()]
  ...
  def get_data_features(self, tokenizer, sequence_length: int = None) -> typing.List[typing.Tuple[str, str, str]]:
    ...
    with self.Session() as session:
      if sequence_length:
        return [(x.sample, x.include, x.feature_vector) for x in session.query(CLSmithSample).filter(CLSmithSample.num_tokens <= sequence_length).limit(1000).offset(0).all()]
      else:
        return [(x.sample, x.include, x.feature_vector) for x in session.query(CLSmithSample).limit(1000).offset(0).all()]
